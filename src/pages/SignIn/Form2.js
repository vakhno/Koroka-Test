	...
	return (
		<form id="form2" onSubmit={handleSubmit(onSubmit)} className="profile__form">
			<div className={`input ${errors.name ? 'input--margin-error' : ''}`}>
				...
				<label className={`${(watchName && !errors.name) || isValid ? "input__pseudoelement-check-mark" : ""}`}>
					<input 
			...
			<div className={`input ${errors.phone ? 'input--margin-error' : ''}`}>
				...
				<label className={`${(watchPhone && !errors.phone) || isValid ? "input__pseudoelement-check-mark" : ""}`}>
					<input 
